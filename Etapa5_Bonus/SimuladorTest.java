    ...
    public static void main(String[] args) {
        ...
        try (Scanner scMove = new Scanner(new File(args[1]))) {
            ...
            while (scMove.hasNext()) {
                ...
                if (accion.equals("Sound")) {
                    String objetivo = scMove.next();
                    for (Celular c : celulares){
                        if (c.nombreDueno.equals(dueno) && c.nombre.equals(equipo) ){
                            for(EloTelTag tag : todosLosRastreables){
                                if (tag.getOwnerName().equals(dueno) && tag.getName().equals(objetivo)){
                                    c.hacerSonar(tag);
                                }
                            }
                        }
                    }
                } else if (accion.equals("FindMy")) {
